...
public class TestdurchfuehrungDAO {
    ...
    public void speichern(Testdurchfuehrung testdurchfuehrung) {
        EntityManager entityManager = EntityManagerProvider.createEntityManager();
        EntityTransaction transaction = entityManager.getTransaction();

        try {
            transaction.begin();
            entityManager.persist(testdurchfuehrung);
            transaction.commit();
    ...
    public List<Testdurchfuehrung> findeFuerTester(Tester tester) {
        EntityManager entityManager = EntityManagerProvider.createEntityManager();

        try {
            return entityManager
                    .createQuery(
                            "SELECT td FROM Testdurchfuehrung td "
    ...
    public void aktualisieren(Testdurchfuehrung testdurchfuehrung) {
        EntityManager entityManager = EntityManagerProvider.createEntityManager();
        EntityTransaction transaction = entityManager.getTransaction();

        try {
            transaction.begin();
            entityManager.merge(testdurchfuehrung);
            transaction.commit();
        } catch (RuntimeException e) {
            if (transaction.isActive()) {
                transaction.rollback();
